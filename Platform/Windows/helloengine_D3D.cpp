...
const uint32_t SCREEN_HEIGHT =  480;//use this to define the screen size
...
struct VERTEX   // vertex buffer structure
{
    XMFLOAT3    Position;
    XMFLOAT4    Color;
};
...
template<class T>
inline void SafeRelease(T **ppInterfaceToRelease)//这个地方在干吗？
{
...
void InitPipeline()// this is the function that loads and prepares the shaders 
{
    ID3DBlob *VS, *PS;  // load and compile the two shaders 
...
void InitGraphics()
{
    ...
    VERTEX OurVertices[] =
    {
        {XMFLOAT3(0.0f, 0.5f, 0.0f), XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f)}, //the first is positon and the second is RGBA
...
HRESULT CreateGraphicsResources(HWND hWnd) //when everything is preparied then begin draw Graphics
{
    HRESULT hr=S_OK;
    if (g_pSwapchain == nullptr)
    {
        ...
        scd.BufferDesc.RefreshRate.Numerator = 60;              // set the refresh rate
        scd.BufferDesc.RefreshRate.Denominator = 1;             // set the refresh denominator
        ...
        hr = D3D11CreateDeviceAndSwapChain(NULL,
                                      ...
                                      &g_pDevcon);
        // so as the book said we need to use success and fail to define the two situation
        if (hr == E_INVALIDARG) 
        {
            hr = D3D11CreateDeviceAndSwapChain(NULL,
                                      D3D_DRIVER_TYPE_HARDWARE,
...
void DiscardGraphicsResources()  // need to judge which resourse need to be release
{
                   ...
                   int nCmdShow)
{
    ...
    //CoUninitialize();// uninitialize COM beacause the D3D are not use the com so doesn't need to dis com
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!wasHandled) { result = DefWindowProc (hWnd, message, wParam, lParam); } //process the 
    return result;
}