...
void GetVideoCardInfo(char* cardName) //to get the video card info
{
...
#pragma region init the VectorType
    ...
    lookAt.y = 0.0f;
    lookAt.z = 1.0f;
#pragma endregion the VectorType
    ...
    BuildViewMatrix(position, lookAt, up, g_viewMatrix);// Finally create the view matrix from the three updated vectors.
}
...
void Draw()
{
    ...
    glClearColor(0.2f, 0.3f, 0.4f, 1.0f);               // Set the color to clear the screen to.
    ...
    // how to make this function more efficitive
    MatrixRotationY(rotationMatrixY, rotateAngle);//use this function to init the matrix with the rotateangle
    MatrixRotationZ(rotationMatrixZ, rotateAngle);
    MatrixMultiply(g_worldMatrix, rotationMatrixZ, rotationMatrixY);

    
    CalculateCameraPosition();// Generate the view matrix based on the camera's position.

    // Set the color shader as the current shader program and set the matrices that it will use for rendering.
    glUseProgram(g_shaderProgram);
    SetShaderParameters(g_worldMatrix, g_viewMatrix, g_projectionMatrix);//wvp matrix
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        {
          Draw();// now you only need to change this function to realize what you want
          return 0;
        }
