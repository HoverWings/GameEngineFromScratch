...
#ifndef ALIGN
#define ALIGN(x, a)         (((x) + ((a) - 1)) & ~((a) - 1))            //Memory Alignment
#endif
...
void My::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
    ...
    size_t minimal_size = (sizeof(BlockHeader) > m_szDataSize) ? sizeof(BlockHeader) : m_szDataSize;//define the minist block size
    ...
    m_nBlocksPerPage = (m_szPageSize - sizeof(PageHeader)) / m_szBlockSize; //only need to control the maxType
}
